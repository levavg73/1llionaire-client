...
export function isRoleAllowedPath(path: string, userType: UserType): boolean {
  if (path.startsWith("/admin")) return userType === "admin";
  if (path.startsWith("/customer")) return userType === "customer";
  if (path.startsWith("/freelancer")) return userType === "freelancer";
  return true;
}
...
export function getPostAuthRedirect(nextPath: string | null | undefined, userType: UserType): string {
  ...
  return isRoleAllowedPath(safeNextPath, userType) ? safeNextPath : fallback;
}
